// List of services: name, image path, description
const SERVICES = [
  {
    name: "Planning",
    img: "assets/service_planning.jpg",
...
// Render Service tiles (still show image in grid, only text in popup)
(function(){
...
// Modal logic (show description only)
function openServiceModal(service) {
  var modal = document.createElement('div');
  modal.className = 'gallery-modal';
  document.body.appendChild(modal);
  modal.innerHTML = `
    <div class="gallery-content" style="align-items:flex-start;max-width:500px;padding:40px 28px;">
      <button class="gallery-close" type="button" style="position:absolute;right:20px;top:16px;background:var(--gold);color:#181A1F;font-size:2rem;width:50px;height:50px;border-radius:50%;border:none;box-shadow:0 6px 20px #0004;cursor:pointer;">×</button>
      <h3 style="color:var(--gold);font-family:'Playfair Display',serif;margin-bottom:18px;font-size:1.22rem;">${service.name}</h3>
  ...
  modal.onclick = function(ev){
    if(ev.target === modal){ if(modal && modal.parentNode) modal.parentNode.removeChild(modal); }
  }
}

// Contact-Us button for services page
